...
def convert_length():
    ...
    length_convert = length_var.get()

    if length_convert == 1:
        result.set(f"{length_value} cm is equal to {length_value / 100} m")
    elif length_convert == 2:
        result.set(f"{length_value} m is equal to {length_value * 100} cm")
    # Add more conversion options here...

# Create a label for the user to select conversion type
label_type = tk.Label(root, text="Select Conversion Type:")
label_type.pack()

# Create a radio button for selecting conversion type (e.g., length)
length_var = tk.IntVar()
length_var.set(1)  # Default to Length
for key, value in length_dict.items():
    rb = tk.Radiobutton(root, text=value, variable=length_var, value=key)
    rb.pack()

# Create an entry widget for the user to enter the value to convert
